...
public class Program
{
    ...
    public static async Task Main(string[] args)
    {
        while (true)
        {
            var clientSocket = new ClientSocket("192.168.1.36", port);
            var _dispatcher = new CommandDispatcher();

            _dispatcher.RegisterCommand("login", new LoginCommand(async (id) =>
            {
                userId = id;
                await DisplayMenu(id, _dispatcher, clientSocket);
            }));
    ...
    static async Task DisplayMenu(int roleId, CommandDispatcher dispatcher, ClientSocket clientSocket)
    {
        switch (roleId)
        {
            ...
            case 2:
                ...
                dispatcher.RegisterCommand("3", new GetRecommendationsCommand());
                dispatcher.RegisterCommand("4", new RolloutMenuCommand());
                dispatcher.RegisterCommand("5", new MonthlyFeedbackReportCommand());
        ...
        while (!exit)
        {
            ...
            else if ((roleId == 1 && commandKey == "5") || (roleId == 2 && commandKey == "6") || (roleId == 3 && commandKey == "3"))
            {
                exit = true;
    ...
    static void DisplayRoleMenu(int roleId)
    {
        switch (roleId)
        {
            ...
            case 2:
                ...
                Console.WriteLine("4) View Monthly Feedback report");
